...
from src.data_loader import clean_prices, compute_returns, load_positions, load_prices
...
def test_compute_returns_generates_expected_values() -> None:
    """compute_returns should return simple pct-change per instrument."""
    prices = pd.DataFrame(
        {
            "date": ["2025-01-01", "2025-01-02", "2025-01-03"] * 2,
            "instrument_id": ["A", "A", "A", "B", "B", "B"],
            "price": [100.0, 110.0, 121.0, 200.0, 180.0, 189.0],
        }
    )
    returns = compute_returns(prices)
    assert list(returns.columns) == ["A", "B"]
    assert len(returns) == 2
    assert abs(float(returns.iloc[0]["A"]) - 0.10) < 1e-12
    assert abs(float(returns.iloc[0]["B"]) - (-0.10)) < 1e-12
    assert abs(float(returns.iloc[1]["A"]) - 0.10) < 1e-12
    assert abs(float(returns.iloc[1]["B"]) - 0.05) < 1e-12


def test_compute_returns_raises_on_missing_columns() -> None:
    """compute_returns should fail fast when required columns are absent."""
    bad_prices = pd.DataFrame({"date": ["2025-01-01"], "instrument_id": ["A"]})
    with pytest.raises(ValueError, match="Missing required price columns"):
        compute_returns(bad_prices)
